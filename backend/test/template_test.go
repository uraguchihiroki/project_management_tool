...
func createTestTemplate(t *testing.T, ts *testServer, projectID, name string, workflowID *string) string {
	t.Helper()
	...
	if workflowID != nil {
		wfIDFloat := float64(0)
		fmt.Sscanf(*workflowID, "%f", &wfIDFloat)
		body["workflow_id"] = uint(wfIDFloat)
	}
...
func TestTemplate_List(t *testing.T) {
	...
	createTestTemplate(t, ts, projectID, "バグ報告", nil)
	createTestTemplate(t, ts, projectID, "機能要望", nil)
...
func TestTemplate_Get(t *testing.T) {
	...
	ownerID := createTestUser(t, ts, "オーナー", "owner@example.com")
	projectID := createTestProject(t, ts, "TM", "テストプロジェクト", ownerID)
	tmplID := createTestTemplate(t, ts, projectID, "取得テスト", nil)
...
func TestTemplate_Update(t *testing.T) {
	...
	ownerID := createTestUser(t, ts, "オーナー", "owner@example.com")
	projectID := createTestProject(t, ts, "TM", "テストプロジェクト", ownerID)
	tmplID := createTestTemplate(t, ts, projectID, "更新前テンプレート", nil)
...
func TestTemplate_Delete(t *testing.T) {
	...
	ownerID := createTestUser(t, ts, "オーナー", "owner@example.com")
	projectID := createTestProject(t, ts, "TM", "テストプロジェクト", ownerID)
	tmplID := createTestTemplate(t, ts, projectID, "削除テスト", nil)
...
func TestTemplate_WithWorkflow(t *testing.T) {
	...
	ownerID := createTestUser(t, ts, "オーナー", "owner@example.com")
	projectID := createTestProject(t, ts, "TM", "テストプロジェクト", ownerID)
	wfID := createTestWorkflow(t, ts, "申請フロー")
	...
	t.Run("ワークフローを紐付けてテンプレートを作成できる", func(t *testing.T) {
		...
		assertStatus(t, status, http.StatusCreated, "create with workflow")
		// workflow_idが保存されているか確認
		data := resp["data"].(map[string]interface{})
		if data["workflow_id"] == nil {
			t.Errorf("workflow_id should not be nil")
		}
	})
}
...
func TestIssue_CreateFromTemplate(t *testing.T) {
	...
	t.Run("テンプレートIDとワークフローIDを指定してIssueを作成できる", func(t *testing.T) {
		...
		data := resp["data"].(map[string]interface{})
		// template_id と workflow_id が保存されているか確認
