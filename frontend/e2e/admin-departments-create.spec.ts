...
test.describe('組織管理者: 部署作成', () => {
  ...
  test('フォームで name 入力 → 作成 → 一覧に表示', async ({ page }) => {
    ...
    const name = `E2E部署${unique}`

    await page.goto('/admin/departments')
    await expect(page.getByRole('heading', { name: /部署/ })).toBeVisible({ timeout: 5000 })

    await page.getByRole('button', { name: /部署を追加|新規/ }).first().click()
