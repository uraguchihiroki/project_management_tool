...
**実装（Go / GORM）:** モデルに `gorm.DeletedAt` を付与し、業務経路の `Delete` は原則ソフト削除とする。**起動時マイグレーション**（`internal/db` の重複除去・レガシー除去など）では意図的に生 `DELETE` や **`Unscoped().Delete`** が残る。多対多の結合テーブル（`user_roles` 等）は **代理 PK（UUID）** により、全差し替えは **既存有効行の論理削除のうえ新行 `Create`** で行い、業務 API では **`Unscoped` に依存しない**（履歴としてソフト削除行が残る）。**`workflow_transitions` / `project_status_transitions`** の遷移の全作り直しなど、単一サロゲート PK でも同一ビジネスキーで再挿入がぶつかる経路では、引き続き **`Unscoped`** による物理削除が残りうる。
