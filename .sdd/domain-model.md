...
- **組織に属する**（workflows.organization_id、組織ごとに管理）
- **存在条件**: ユーザー作成ステータス（status_key が空）を持つステップが1つ以上ある場合のみ「存在する」とみなす。sts_start と sts_goal のみのワークフローは存在しない。
...
| 項目 | 説明 |
|------|------|
| **現在のステータス** | 承認ステップの状態（未承認 / 承認済み） |
| **ステップの説明** | ステップの説明文 |
| **承認オブジェクト** | 承認条件を満たす対象のリスト（1:N） |
| **閾値** | 承認後ステータスへ遷移するための合計点数 |
| **承認後ステータス** | ステップ完了時に Issue が遷移する先のステータス。ユーザーは直接編集不可。ワークフロー詳細の並び替え保存で確定される |

### 承認オブジェクト（Approval Object）が持つもの
...
|------|------|
| 役職 | 対象となる役職（Role） |
| 役職「比較演算子」 | イコール（=）: 役職 level が完全一致した場合のみ承認可能。以上（>=）: 指定 level 以上の役職が承認可能 |
| 点数 | 承認時に加算される点数 |
...
| 項目 | 説明 |
|------|------|
| exclude_reporter | 起票者をこの承認オブジェクトの承認者から除外する |
| exclude_assignee | 担当者をこの承認オブジェクトの承認者から除外する |

### ステップと承認オブジェクトの関係

- **1 : N** — 1 ステップは複数の承認オブジェクトを持つ

### 承認後ステータスへ遷移する条件

- **承認オブジェクトの点数の合計が閾値以上（`>=`）になった時** に遷移する

### 承認オブジェクトの点数ルール

- **同一人物は同一ステップでは 1 回しか承認できない**
- **同一人物が複数の承認オブジェクトに該当する場合** — 点数が最も高い承認オブジェクトの点数で 1 回のみ加算

### ワークフローの構造

- ワークフローは必ず **「スタート」ステータス（sts_start）** をもつステップから始まり、**「ゴール」ステータス（sts_goal）** をもつステップで終わる
- 各ワークフローに start ステップ・goal ステップはそれぞれ 1 つずつ存在する
- **sts_start / sts_goal のステップは画面に表示しない**（ステータス一覧・ステップ一覧・ステータス選択から除外）
- **sts_start ステップ**: order=0。承認後ステータス（next_status_id）のみ有効で、それ以外の項目は基本的に参照されない
- **sts_goal ステップ**: order=ワークフローの総ステップ数-1。他の項目は無効で基本的に参照されない
- **「ゴール」ステータス** をもつステップは、ステップの説明以外は無効（編集不可・グレーアウト）
  - 承認オブジェクト、閾値、承認後ステータスは設定しない（UI 上グレーアウト）
- **初回ステップ追加時**: ユーザーが最初のステップ（ユーザー作成ステータス）を追加したときに、sts_start と sts_goal のステップを自動で先頭・末尾に作成する
- **承認後ステータスの編集**: 「ステップを編集」画面では承認後ステータスは表示されない。ユーザーは直接編集できない
- **ワークフロー詳細での並び替え**: ステップは自由に並べ替え可能。保存ボタン押下時に承認後ステータスが確定する（並び順に応じて next_status_id が自動設定される）

### 既知の制限（2025-03-17）

> **今現在においては、別の部署の承認者が承認できてしまうが、仕様とする。（Issue に部署情報がのるまで）**
...
| Workflow | project_id（プロジェクトに紐づく） | グローバル（組織に属さない） |
| WorkflowStep | 役職（required_level）のみ | 承認オブジェクト（1:N）、閾値、点数ベースの遷移 |
...
| 用語 | 意味 | 新仕様での扱い |
|------|------|----------------|
| **min_approvers** | 同一ステップで「最低 N 人から承認が必要」という条件 | **不要**。閾値で表現可能（例: 承認オブジェクト 3 つ×10 点、閾値 20 点以上 → 2 人以上で同じ） |
| **exclude_reporter** | 起票者を承認者から除外 | **必要**。承認オブジェクトごとに持つ |
| **exclude_assignee** | 担当者を承認者から除外 | **必要**。承認オブジェクトごとに持つ |
