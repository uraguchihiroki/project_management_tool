...
新規組織（およびプロジェクト用デフォルト Issue ワークフロー作成時）の初期シードは、表示名 **未着手・進行・完了** の 3 ステータスまでとする。**許可遷移は初期シードでは作らず**、利用者が API／管理画面で都度追加する。プロジェクト進行用の `project_statuses` も同様に、デフォルト列のみ投入し **`project_status_transitions` は初期投入しない**。
