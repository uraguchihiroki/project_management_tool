...
| 結合テーブル | `AssignRolesToUser` / 部署の `SetUserDepartments` は **論理削除＋新行 Create**（代理 PK により `Unscoped` 不要）。重複 ID は入力側で除去。 | done |
| organization_id 絶対化 | 結合テーブル `user_roles` / `organization_user_departments` に `organization_id` を保持し、作成時に埋める。既存データは起動時マイグレーションで backfill。 | done |
